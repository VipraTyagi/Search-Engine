...
class Spider:
    ...
    def add_url_to_visit(self, url):
        if url not in self.visited_urls and url not in self.urls_to_visit:
            ...
            if re.match(r"https://stuffyoushouldknow\.com/.[a-zA-Z0-9-_%=]*", str(url)) and 'timestamp' not in str(url):
    ...
    def get_content(self, link):
        try:
            ...
            notes = soup.find_all('li', class_='menu-item menu-item-type-post_type menu-item-object-page menu-item-480')
            description = ''
            for n in notes:
                if n.find('p') is not None:
                    description = n.find('p').text          
            topic = soup.find('a', class_='menu-item menu-item-type-post_type menu-item-object-page menu-item-480')
            podcast = {
                'title': title,
                'link': link,
                'topic': topic,
    ...
    def crawl(self, url):
        ...
        for linked_url in self.get_linked_urls(url, html):
            if re.match(r"https://stuffyoushouldknow\.com/.[a-zA-Z0-9-_%=]*", str(linked_url)) and 'timestamp' not in str(linked_url):
